...
for repeat_time in ['1', '2', '3']:
    for model_name in model_size:
        for tissue in tissue_list:
            ...
            image_path = 'images/' + embedding_name + '/slope/shuffled/'

            train_data = pd.read_pickle(data_path + embedding_name + '_' + model_name + '_train.dataset')
            valid_data = pd.read_pickle(data_path + embedding_name + '_' + model_name + '_valid.dataset')
            test_data = pd.read_pickle(data_path + embedding_name + '_' + model_name + '_test.dataset')
            ...
            with open(
                    image_path + model_name + '-performance-' + tissue + '(' + task_name + ')_' + repeat_time + '.txt',
                    'w') as f:
            ...
            sns.regplot(x=outputs, y=Y_test_np, x_jitter=0.15, y_jitter=0.15,
                        scatter_kws={'color': '#2E8B57', 'alpha': 0.7, 's': 15}, line_kws={"color": "#006400"},
                        truncate=False)
            plt.title(model_name + ' Model (' + task_name + ')\n' + tissue + ', ' + 'Transformer')

            plt.savefig(
                image_path + model_name + '-transformer-' + tissue + '(' + task_name + ')_' + repeat_time + '.png',
